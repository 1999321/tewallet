...
export class WalletsListManager {
    ...
    private async fetchWalletsList(): Promise<WalletInfo[]> {
        ...
        try {
            ...
            const wrongFormatWallets:any[] = walletsList.filter(
    ...
    private isCorrectWalletConfigDTO(value: any): value is WalletInfoDTO {
        ...
        const sseBridge:any = bridge.find(item => (item as { type: string }).type === 'sse');
        ...
        const jsBridge:any = bridge.find(item => (item as { type: string }).type === 'js');
